...
# Ajustar Heligman–Pollard en el tramo “estable”
fit_HP <- MortalityLaw(
  x  = base2$edad,
  qx = base2$TasaBruta,
  law = "HP"
...
# predecir qx HP para todo el rango
edad_all <- 0:109
qx_HP <- predict(fit_HP, x = edad_all)
...
tabla_final <- tabla_HP %>%
  left_join(base2 %>% select(edad, qx_obs = TasaBruta), by = "edad") %>%
  mutate(
    qx_extrap = case_when(
      edad >= 20 & edad < 85 ~ qx_obs,  # tramo observado
      TRUE                   ~ qx_HP   # cabeza y cola del modelo
    )
  ) %>%
  arrange(edad)

plot(tabla_final$edad, tabla_final$qx_extrap,
     type = "p", pch = 19, col = "gray40",
     xlab = "Edad", ylab = "Tasa de mortalidad",
     main = "Extrapolación Heligman–Pollard")
lines(tabla_final$edad[0:10], tabla_final$qx_extrap[0:10], lwd = 3, col="red")
lines(tabla_final$edad[85:109], tabla_final$qx_extrap[85:109], lwd = 3, col="red")
...
tabla_extrapolada <- tabla_final %>%
  select(edad, qx = qx_extrap)

library(openxlsx)
write.xlsx(tabla_extrapolada, file = "tabla_extrapolada.xlsx")
# me di cuenta que faltan los datos de 0 a 10